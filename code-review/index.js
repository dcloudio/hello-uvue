...
function loadProjectConfig() {
  try {
    ...
    return {
      ...
      geminiEndpoint: config.project.geminiEndpoint || 'ablai'
    };
  } catch (error) {
...
async function getAIReview(prompt, projectConfig) {
  try {
    ...
    const geminiEndpoint = projectConfig.geminiEndpoint || 'ablai';
    ...
    // 创建 axios 实例，仅 Gemini 模型使用代理
    const axiosInstance = axios.create({
      proxy: isGemini && geminiEndpoint === 'google' ? {
        protocol: 'http',
        host: HTTP_PROXY?.split(':')[0] || HTTPS_PROXY?.split(':')[0],
        port: HTTP_PROXY?.split(':')[1] || HTTPS_PROXY?.split(':')[1]
      } : false,
    ...
    if (isGemini) {
      if (geminiEndpoint === 'google') {
        ...
        if (!apiKey) {
          throw new Error('ABLAI_API_KEY 未设置');
        }
      }
    } else {
      ...
      if (!apiKey) {
        throw new Error('BAILIAN_API_KEY 未设置');
      }
    }

    // 使用重试机制发送请求
    const response = await retryWithDelay(async () => {
      return await axiosInstance.post(apiUrl, {
        model: model,
